				...
				if (children) { //Has children, so descends the tree
					...
					if ((int)contents.size() > maxSize && depthLeft > 0) {
						if (!children) { //Create new children if full
							...
							contents.clear(); //Remove its own contents as they are now in its child nodes
						}
					}
				}
			}
